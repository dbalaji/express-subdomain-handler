...
function subdomainRegex(baseUrl){
    ...
    if( Object.prototype.toString.call( baseUrl ) === '[object Array]' ) {
        baseUrlRegExp = "(";
        for (var index= 0, numUrls= baseUrl.length; index < numUrls; index++){
            if (index>0){
                baseUrlRegExp += "|";
            }
            baseUrlRegExp += _escapeRegExp(baseUrl[index]);
        }
        baseUrlRegExp += ")";
    }
    else {
        baseUrlRegExp = _escapeRegExp(baseUrl);
    }

	regex = new RegExp('((?!www)\\b[\\w\\.]+)\\.' + baseUrlRegExp + '(?::)?(?:\d+)?');
...
module.exports = function(options){
	...
	// prefix default
	options.prefix = typeof(options.prefix) === "undefined" ?  'subdomain' : options.prefix;
	...
	return function(req, res, next){
		...
		req.url = (options.prefix ? '/' + options.prefix : '')+ req.url;
